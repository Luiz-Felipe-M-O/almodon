namespace jsxmm {
	...
	export function Element<T extends keyof HTMLElementTagNameMap>(tag: T, properties: Properties<T> = {}, ...children: (Node | Component | string)[]): HTMLElementTagNameMap[T] {
		const element = document.createElement(tag)
		replace(element, properties)

		for (let i = 0; i < children.length; i++) {
			const child = children[i]
			if (typeof child === "object" && Object.hasOwn(child, "HTML") && typeof (child as any).HTML === "function") {
				element.append((child as Component).HTML())
			} else {
